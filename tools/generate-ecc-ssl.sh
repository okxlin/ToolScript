...
read -p "请输入SSL证书有效期（默认为100年）: " ssl_days
...
# 创建CA私钥（使用ECDSA算法）
openssl genpkey -algorithm EC -pkeyopt ec_paramgen_curve:P-256 -out "$cert_dir/CA.key"
...
# 创建证书的ECDSA私钥
openssl genpkey -algorithm EC -pkeyopt ec_paramgen_curve:P-256 -out "$cert_dir/private.key"
