...
# 函数：创建目录
create_dir() {
    local panel_dir=~/1panel-install-dir
    mkdir -p "${panel_dir}"
    cd "${panel_dir}" || { echo "无法进入目录 ${panel_dir}，脚本无法继续执行。"; exit 1; }
}
...
remove_container() {
    ...
    while [[ $container_exists == false ]]; do
        ...
        read -p "请输入要移除的 1Panel 容器名（默认为 '1panel'；输入 'removed' 则表示已经手动移除过，将跳过移除）: " container_name
